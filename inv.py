...
class invader():
    def __init__(self):
        #アイコン
        self.giv1 = pygame.image.load("img/invader1.png").convert_alpha()#インベーダー　　50x50
        self.giv2 = pygame.image.load("img/invader2.png").convert_alpha()#インベーダー２　50x50

        self.ct=0    #カウンタ　10fps(fps_ct) になるごとにインベーダを動かす
        self.fps_ct=0  #何カウントで動かすか
        self.anm_inv=0 #インベーダの手の上げ下げ
        ...
        self.min=0
        self.max=7
        self.fps_ct=10#なんfpsごとにインベーダを動かすか
    ...
    def calcinvader(self):
        ...
        self.ct += 1
        if self.ct > self.fps_ct:#内部クロック10フレームに一回、全体を動かす
            self.ct=0
            self.xinv += self.vinv
        ...
        #インベーダ行列の描画　動かすのは10カウントに1回だが描画は毎回やる
        #　そうしないと消えた感じがする
        self.drawinvaders()
    ...
    def drawinvaders(self):
      for y in range (4):#横5行
        for x in range (8):#縦8個
          #インベーダ行列の中で、x,yのインベーダが生きているなら以下実行
...
class player():
    #砲台　　　　　　　50x50
    gply = pygame.image.load("img/player.png").convert_alpha()
    vply=0
    xply=0
    yply=0
    left_ply=0
    ...
    def calcplayer(self,fl):
        ...
        #あげミサイル発射
        #スペースキーが押され　fspky=1　かつ上げミサイルが未発射　fagm==0　なら
...
class agm(disp):#()内は引数じゃなくて親クラス　dispクラスのメソッドを使う
    #agmミサイル　　　　　20x50
    gagm = pygame.image.load("img/age.png").convert_alpha()
    xagm=0
    yagm=0
...
class shelter():
    #シェルター　　　　20x20
    gsh1 = pygame.image.load("img/sh_blk.png").convert_alpha()
    gsh2 = pygame.image.load("img/sh_blk2.png").convert_alpha()#残骸
    gsh3 = pygame.image.load("img/sh_blk3.png").convert_alpha()#残骸
    gsh4 = pygame.image.load("img/sh_blk4.png").convert_alpha()#残骸

    #シェルター描画　４つ分
    #shelter　当たり判定
    def drawshelters(self):
...
def del_sgm(sg,pl):
    ...
    #http://localhost:8888/notebooks/reversed%E3%82%92%E4%BD%BF%E3%81%86%E7%90%86%E7%94%B1.ipynb
...
def main():
    ...
    while True:#砲台1つ分のループ
        ...
        pl.left_ply -=1
        if pl.left_ply ==0:
            break#あなたの負けでループを抜ける
    #----外側のwhile True:の最後　（砲台１つぶん）

    saisyu_hantei(iv,pl,ck)#最後の判定
